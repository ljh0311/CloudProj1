...
export async function getProductById(id) {
    const result = await executeQuery(
        'SELECT *, CAST(price AS DECIMAL(10,2)) as price FROM products WHERE id = ?',
        [id]
    );
...
export async function addProduct(productData) {
    const result = await executeQuery(
        ...
        [
            ...
            productData.size_s_stock || 20,
            productData.size_m_stock || 20,
            productData.size_l_stock || 20
        ]
    );
...
export async function getUsers() {
    const result = await executeQuery('SELECT id, name, email, role, created_at, updated_at FROM users');
    if (!result.success) {
